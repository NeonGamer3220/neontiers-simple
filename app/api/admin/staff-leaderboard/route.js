// app/api/admin/staff-leaderboard/route.js
...
//
// Deliberately a separate route from /api/admin/tester-stats, which
// powers the unrelated "Teszterek és tesztek" panel (players marked as
// testers via tests.is_tester) — the two were colliding on one shape
// before this split.
...
export async function GET() {
  ...
  for (const row of testsRes.data || []) {
    ...
    if (!byTester.has(key)) {
      byTester.set(key, { username: rawId, total: 0, week: 0, modes: new Set() });
    }
  ...
  const sortDesc = (a, b) => b.week - a.week || b.total - a.total || a.username.localeCompare(b.username);
