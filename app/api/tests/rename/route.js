...
export async function POST(req) {
  ...
  // Keep linked_accounts in sync too — Ban Kezelő and the High-Test manager
  // search this table by Minecraft name, so without this the renamed player
  // would keep showing up under their old name (or not be found at all).
  const MC_NAME_COLUMNS = ["minecraft_name", "minecraft_username", "mc_name", "ign", "username", "minecraft"];
  try {
    const { data: linkedRows } = await supabase.from("linked_accounts").select("*").limit(1000);
    if (Array.isArray(linkedRows)) {
      for (const row of linkedRows) {
        for (const col of MC_NAME_COLUMNS) {
          if (row[col] && String(row[col]).trim().toLowerCase() === oldName.toLowerCase()) {
            await supabase.from("linked_accounts").update({ [col]: newName }).eq("id", row.id);
            break;
          }
        }
      }
    ...
    console.error("Failed to sync linked_accounts on rename:", e?.message || e);
  }
